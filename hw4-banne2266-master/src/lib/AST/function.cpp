...
const char *FunctionNode::getParatypeCString() const {
    if (!paratype_string_is_valid) {
        for (const auto &parameter : parameters) {
            for (const auto &var : parameter->variables()) {
                paratype_string.append(var->getTypeCString()).append(", ");
            }
        }

        if (!parameters.empty()) {
            // remove the trailing ", "
            paratype_string.erase(paratype_string.end() - 2,
                                   paratype_string.end());
        }

        paratype_string_is_valid = true;
    }

    return paratype_string.c_str();
}
